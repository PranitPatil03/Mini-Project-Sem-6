...
def build_model(input_shape):
    ...
    cnn_lstm_model.compile(optimizer='adam', loss='mean_squared_logarithmic_error')
...
if len(x_forcast) > 0:
    ...
    adj_close_prediction = arima_model_for_Adj_Close_twitter.predict(n_periods=len(x_forcast),exogenous=x_forcast)
    ...
    if len(open_prediction) > 1:
        print(f'Date = {forecast_dates[-2]}, Prediction open {open_prediction[1]}')
    else:
        print("Insufficient data for prediction.")
    if len(adj_close_prediction) > 1:
        print(f'Date = {forecast_dates[-2]}, Prediction Adjusted close {adj_close_prediction[1]}')
    else:
        print("Insufficient data for prediction.")
else:
    print("Insufficient data for prediction.")
